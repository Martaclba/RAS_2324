...
function ProvasDocente() {
    ...
    useEffect(() => {
        ...
        axios.get('http://localhost:7778/provas/aluno/pg53895')
            .then(response => {
    ...
    const realizar = (id,nome) => {
        fetch(`http://localhost:7778/provas/${id}/versao/1/questoes`, {
          ...
          .then(questoes => {
            ...
            console.log(id)
            history('/realizarProva2',{ state: {questoes,indice:0, respostas : respostas, nome,id} });
                                        ...
                                        <tr key={prova.id_prova}>
                                            ...
                                            <td className="mt-6 flex items-center justify-end gap-x-6">
                                                {prova.estado === 'REALIZAR' && (
                                                    <button className="btn btn-sm" type="button" onClick={() => realizar(prova.id_prova,prova.nome)}>
                                                        Realizar
                                                    </button>
                                                )}
                                                {prova.estado === 'CORRIGIDA' && (
                                                    <button className="btn btn-sm" type="button" onClick={() => consultar(prova.id_prova,prova.nome)}>
                                                        Consultar
                                                    </button>
                                                )}
                                            </td>
